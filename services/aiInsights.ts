...
/**
 * Get a personalized savings tip based on spending patterns
 */
export function getSavingsTip(transactions: Transaction[]): string | null {
  const insights = generateSmartInsights(transactions);

  if (insights.length === 0) {
    return 'Śledź swoje wydatki regularnie, aby otrzymywać personalizowane porady!';
  }
  ...
  switch (topInsight.category) {
    case 'food':
      return 'Spróbuj gotować w domu zamiast jedzenia na mieście - możesz zaoszczędzić do 50%!';
    case 'transport':
      return 'Rozważ zakup biletu miesięcznego - może być tańszy niż pojedyncze przejazdy!';
    case 'entertainment':
      return 'Szukaj darmowych wydarzeń w Twojej okolicy - wiele muzeów ma dni otwarte!';
    case 'shopping':
      return 'Przed zakupem poczekaj 24h - często okaże się, że nie potrzebujesz tego produktu!';
    default:
      return 'Ustaw budżet miesięczny dla każdej kategorii i staraj się go trzymać!';
  }
}

/**
 * Analyze if user should be alerted about overspending
 */
export function checkOverspendingAlert(
  ...
  for (const [categoryId, spent] of categorySpending) {
    ...
    if (budget && spent > budget) {
      ...
      return {
        alert: true,
        message: `Przekroczyłeś budżet dla ${category?.name || 'kategorii'} o ${overspend.toFixed(2)} PLN!`,
      };
    }
  }
